...
export class WeightKnob extends LitElement {
  ...
  override render() {
    ...
    // Intensity of the glow effect is based on prompt weight and audio level.
    const glowIntensity = this.value / 2 + this.audioLevel;

    const haloStyle = styleMap({
      display: this.value > 0 ? 'block' : 'none',
      // Use a radial gradient for a "hot core" and a box-shadow for the outer glow.
      background: `radial-gradient(circle, white 5%, ${this.color} 30%, transparent 60%)`,
      boxShadow: `0 0 ${5 + 20 * glowIntensity}px ${this.color}`,
      transform: `scale(${scale})`,
      // Opacity is also tied to the intensity to create a fade-in effect.
      opacity: `${Math.min(1, glowIntensity * 0.75)}`,
    });
